    ...
    <>
      <section className='grid grid-cols-1 md:grid-cols-2 gap-2 w-full' id='home'>
        <div className='py-2 flex-1 flex flex-col items-start justify-center gap-6'>
          ...
          <p className='text-base text-textColor text-center md:text-left md:w-[80%]'>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Possimus ex voluptatem dolore, voluptatum dignissimos sit laborum ab minima reprehenderit, impedit exercitationem. Dicta provident inventore quo blanditiis adipisci, doloribus dignissimos saepe!</p>
            ...
            {heroData &&
              heroData.map((n) => (
                <div key={n.id} className='lg:w-190 p-4 bg-cardOverlay backdrop-blur-md rounded-3xl flex items-center justify-center flex-col '>
                  <img src={n.image} className='w-20 lg:w-40 -mt-10 lg:-mt-16' alt='home1' />
                  <p className='text-base lg:text-xl font-semibold text-textColor mt-2 lg:mt-3'>{n.name}</p>
                  <p className='text-[10px] lg:text-sm text-lighttextGray font-semibold my-1 lg:my-2'>{n.desc}</p>
