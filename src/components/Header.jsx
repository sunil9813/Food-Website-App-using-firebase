...
import { BsBasket2Fill, BsPlus } from "react-icons/bs"
    ...
    <header className='fixed z-50 w-screen p-3 px-4 md:p-6 md:px-16 bg-primary'>
      ...
      <div className='hidden md:flex h-full w-full items-center justify-between'>
        ...
        <div className='flex items-center gap-8'>
          {/* navlinks  */}
          <motion.ul exit={{ opacity: 0, x: 200 }} animate={{ opacity: 1, x: 0 }} initial={{ opacity: 0, x: 200 }} className='flex items-center gap-8'>
          ...
          <div className='relative flex items-center justify-center' onClick={showCart}>
            ...
            {/* yo div chai render huna parxa yadi card items xa bahne matra */}
          ...
          <div className='relative'>
            <motion.img whileTap={{ scale: 0.6 }} src={user ? user.photoURL : avatar} className='w-10 min-w-[40px] h-10 min-h-[40px] drop-shadow-xl cursor-pointer' alt='user' onClick={login} />
            {isMenu && (
              <motion.div
                initial={{
                  opacity: 0,
              ...
              >
                {user && user.email === "sunilpokhrel797@gmail.com" && (
                  <Link to={"/createItem"}>
      ...
      <div className='flex items-center justify-between md:hidden h-full'>
        ...
        {/* cart info */}
        <div className='relative flex items-center justify-center float-right' onClick={showCart}>
          <BsBasket2Fill className='text-textColor text-2xl cursor-pointer' />
          {cartItems && cartItems.length > 0 && (
            <div className='absolute -top-2 -right-2 w-5 h-5 rounded-full bg-cartNumBg flex items-center justify-center'>
              <p className='text-xs text-white font-semibold'>{cartItems.length}</p>
          ...
          {isMenu && (
            <motion.div initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.6 }} className='w-40 bg-gray-50 shadow-xl rounded-lg flex flex-col absolute top-12 right-0  '>
              ...
              <ul className='flex flex-col'>
                ...
                <li className='text-base text-textColor hover:text-headingColor  hover:bg-slate-100 px-4 py-2 duration-100 transition-all ease-in-out cursor-pointer'>Services</li>
              </ul>
              <p className='m-2 p-2 rounded-md shadow-md flex items-center justify-center bg-slate-200  gap-3 cursor-pointer hover:bg-slate-300 transition-all duration-100 ease-in-out text-textColor text-base' onClick={logout}>
                Logout <FiLogIn />
              </p>
            </motion.div>
          )}
